      ...
      <div className="landing-header">
        <header className="site-header min-h-16 flex items-center justify-between gap-3 py-3 sm:py-0 sm:h-16 sm:px-8 lg:px-12 border-b border-line">
          ...
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            <Link href="/login" className="hidden md:inline-flex h-9 px-3 sm:px-4 items-center rounded-full text-sm text-zinc-300 hover:text-white transition">Log in</Link>
            <Link href="/login?mode=create" className="bg-[#6EE7D8] text-ink hidden md:inline-flex h-9 px-4 items-center rounded-full text-sm font-semibold hover:opacity-90 transition">Get started</Link>
            <button 
              type="button" 
        ...
        {isMobileMenuOpen && (
          <nav id="site-nav" className="md:hidden border-b border-line bg-panel p-4 flex flex-col gap-4 text-sm" aria-label="Mobile">
            ...
            <Link href="/blog" onClick={() => setIsMobileMenuOpen(false)}>Guides</Link>
            <hr className="border-line" />
            <Link href="/login" onClick={() => setIsMobileMenuOpen(false)}>Log in</Link>
            <Link href="/login?mode=create" className="text-[#6EE7D8] font-semibold" onClick={() => setIsMobileMenuOpen(false)}>Get started</Link>
          </nav>
        )}
      </div>
        ...
        <section className="px-5 sm:px-8 lg:px-12 pt-28 sm:pt-40 pb-8">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-3 mb-12">
              <Link href="/login?mode=create" className="bg-[#6EE7D8] text-ink hover:opacity-90 transition h-12 px-8 inline-flex items-center justify-center rounded-full text-sm font-semibold w-full sm:w-auto">Get started</Link>
              <Link href="/login" className="h-12 px-8 inline-flex items-center justify-center rounded-full border border-line text-sm text-zinc-300 hover:text-white hover:border-zinc-500 transition w-full sm:w-auto">Log in</Link>
            </div>
          </div>
        </section>
        ...
        <section id="product" className="px-5 sm:px-8 lg:px-12 pb-24">
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <Link href="/login?mode=create" className="group bg-panel border border-line rounded-3xl overflow-hidden hover:border-zinc-600 transition block">
        ...
        <section id="guides" className="px-5 sm:px-8 lg:px-12 pb-24" aria-labelledby="guides-heading">
          <div className="max-w-6xl mx-auto">
            <div className="flex items-end justify-between gap-4 mb-8">
              ...
              <Link href="/blog" className="text-sm text-[#6EE7D8] hover:underline shrink-0">All guides</Link>
            </div>
            <div id="home-posts" className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {posts.map((p) => (
                <Link key={p.slug} href={`/blog/${p.slug}`} className="bg-panel border border-line rounded-2xl p-6 hover:border-zinc-600 transition block">
        ...
        <section className="px-5 sm:px-8 lg:px-12 pb-24">
          <div className="max-w-6xl mx-auto relative overflow-hidden rounded-3xl px-6 sm:px-16 py-16 sm:py-24 text-center border border-line">
            ...
            <div className="relative">
              ...
              <Link href="/login?mode=create" className="bg-[#6EE7D8] text-ink hover:opacity-90 transition inline-flex items-center justify-center h-12 px-8 rounded-full text-sm font-semibold">Get started</Link>
            </div>
          </div>
        </section>
      </main>
      ...
      <footer className="border-t border-line px-5 sm:px-8 lg:px-12 py-12">
        <div className="max-w-6xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-8 mb-10">
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm text-zinc-400">
              <li><Link href="/blog" className="hover:text-white transition">Guides</Link></li>
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm text-zinc-400">
              <li><Link href="/login?mode=create" className="hover:text-white transition">Sign up</Link></li>
              <li><Link href="/login" className="hover:text-white transition">Log in</Link></li>
            </ul>
          </div>
        </div>
